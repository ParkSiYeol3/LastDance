...
import { View, TextInput, FlatList, Text, TouchableOpacity, Image, StyleSheet, Alert } from 'react-native';
...
const ChatRoom = ({ route }) => {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (room.me?.uid) {
					map[room.me.uid] = {
						profileImage: typeof room.me.profileImage === 'string' ? room.me.profileImage.replace(/^"|"$/g, '') : null,
				...
				if (room.opponent?.uid) {
					map[room.opponent.uid] = {
						profileImage: typeof room.opponent.profileImage === 'string' ? room.opponent.profileImage.replace(/^"|"$/g, '') : null,
	...
	useEffect(() => {
		const ready = typeof isSeller === 'boolean' && rentalItemId && ((isSeller && buyerId) || (!isSeller && userId));
	...
	useFocusEffect(
		useCallback(() => {
			if (typeof isSeller === 'boolean' && rentalItemId && ((isSeller && buyerId) || (!isSeller && userId))) {
				checkReviewSubmitted();
			}
	...
	const renderItem = ({ item }) => {
		...
		return (
			<View style={[styles.row, isMe ? styles.rowRight : styles.rowLeft]}>
				{!isMe && (
					<Image
						source={typeof profile.profileImage === 'string' && profile.profileImage.startsWith('http') ? { uri: profile.profileImage } : require('../assets/profile.png')}
						style={styles.avatar}
					/>
				)}
	...
	return (
		<View style={styles.container}>
			{paymentStatus === 'refunded' && !reviewSubmitted && (
				<TouchableOpacity style={{ backgroundColor: '#6a5acd', padding: 12, margin: 10, borderRadius: 6 }} onPress={handleNavigateToReview}>
					<Text style={{ color: '#fff', textAlign: 'center', fontWeight: 'bold' }}>📝 거래 후기 작성하기</Text>
				</TouchableOpacity>
			)}
			...
			<FlatList data={messages} inverted keyExtractor={(item) => item.id} renderItem={renderItem} contentContainerStyle={{ padding: 10 }} />
